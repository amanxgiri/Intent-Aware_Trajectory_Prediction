...
async def lifespan(app: FastAPI):
    ...
    if os.path.exists(CHECKPOINT_PATH):
        ...
        print(f"Warning: No checkpoint found at {CHECKPOINT_PATH}. Using untrained parameters for Demo Mode!")
...
def predict_trajectory(request: InferenceRequest):
    ...
    try:
        ...
        neighbors_tensor = torch.tensor(request.neighbors, dtype=torch.float32, device=device)
    ...
    with torch.no_grad():
        try:
            ...
            mode_probabilities = F.softmax(mode_logits, dim=-1)
             ...
             raise HTTPException(status_code=422, detail=f"Model shape validation failed: {ve}")
        except Exception as e:
