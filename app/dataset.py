...
class NuScenesTrajectoryDataset(Dataset):
    ...
    def __init__(
        ...
        try:
            self.nusc = NuScenes(
                version=self.version, dataroot=self.dataroot, verbose=False
            )
        except Exception as exc:
            available_text = (
                ", ".join(available_versions) if available_versions else "none detected"
            )
            raise ValueError(
    ...
    def _detect_available_versions(dataroot: str) -> List[str]:
        ...
        for child in root.iterdir():
            if (
                child.is_dir()
                and child.name.startswith("v1.0-")
                and (child / "sample.json").exists()
            ):
    ...
    def _is_allowed_agent(self, ann: Dict) -> bool:
        ...
        return any(
            category.startswith(prefix) for prefix in self.allowed_category_prefixes
        )
    ...
    def _get_velocity_xy(self, ann_token: str) -> np.ndarray:
        ...
        if vel_xyz is None or np.any(np.isnan(vel_xyz)):
            ...
            if ann["prev"] != "":
                ...
                t0 = (
                    float(self._get_sample(prev_ann["sample_token"])["timestamp"])
                    / 1_000_000.0
                )
                t1 = (
                    float(self._get_sample(ann["sample_token"])["timestamp"])
                    / 1_000_000.0
                )
                dt = max(t1 - t0, 1e-3)
                vx = (
                    float(ann["translation"][0]) - float(prev_ann["translation"][0])
                ) / dt
                vy = (
                    float(ann["translation"][1]) - float(prev_ann["translation"][1])
                ) / dt
    ...
    def _build_index(self) -> List[str]:
        ...
        if not split_scenes:
            raise ValueError(
                f"Unknown split '{self.split}'. Use one of: train, val, test, mini_train, mini_val."
            )

        valid_scene_tokens = {
            scene["token"] for scene in self.nusc.scene if scene["name"] in split_scenes
        }
        items: List[str] = []
    ...
    def __getitem__(self, idx: int) -> Dict[str, Tensor]:
        ...
        agent_past, target_future = self._get_agent_trajectory(
    ...
    def _get_agent_trajectory(self, sample_token, instance_token):
        current_ann_token = self._sample_instance_to_ann.get(sample_token, {}).get(
            instance_token, ""
        )
    ...
    def _get_neighbors(self, sample_token, agent_x, agent_y):
        current_ann_token = self._sample_instance_to_ann[sample_token][
            self._current_instance_token
        ]
        agent_yaw = self._get_agent_yaw(current_ann_token)
        ...
        neighbors = torch.zeros(
            (self.max_neighbors, self.t_past, 4), dtype=torch.float32
        )
        ...
        if location not in self._map_cache:
            try:
                self._map_cache[location] = NuScenesMap(
                    dataroot=self.dataroot, map_name=location
                )
            except Exception:
    ...
    def _get_map_crop(self, sample_token, agent_x, agent_y):
        ...
        if self._current_instance_token is not None:
            agent_tok = self._sample_instance_to_ann.get(sample_token, {}).get(
                self._current_instance_token, ""
            )
            if agent_tok != "":
        ...
        if map_mask.shape[1] != h or map_mask.shape[2] != w:
            ...
            for c in range(map_mask.shape[0]):
                resized.append(
                    cv2.resize(map_mask[c], (w, h), interpolation=cv2.INTER_NEAREST)
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="nuScenes trajectory dataset sanity check"
    )
    parser.add_argument(
        "--dataroot", required=True, help="Path to nuScenes root directory"
    )
    parser.add_argument(
        "--version", default="v1.0-mini", help="nuScenes version folder name"
    )
    parser.add_argument(
        "--split",
        default="train",
        help="Split name (train/val/test or mini_train/mini_val)",
    )
